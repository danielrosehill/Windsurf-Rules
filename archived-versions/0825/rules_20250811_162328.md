...
*Rules constructed on 2025-08-11 16:24:19 using intelligent LLM organization*
*Organization strategy: Present foundational user and system context first (who Daniel is, hardware, and LAN layout), then environment and infrastructure constraints (Cloudflare, containers, MCPs). Next, enumerate tools, stack preferences, and LLM selection rules that govern runtime behavior. Follow with prioritization and workflow rules that dictate decision-making and execution. End with documentation and speech-to-text handling notes. This ordering supports quick orientation, enforces critical constraints early, and places operational rules where they're actionable.*